<?php
	...
	//接收上传的实验数据
		...
		$result = $db -> query("SELECT `E_std` FROM `physics_course_potentioneter` `group_num`={$group_num}");		//获取老师输入的实验预期值
		$data = $result -> fetch_assoc();
		$E_e = abs(($_GET['E']-$data['E_std'])) / $data['E_std'];		//计算实验误差
		$E_e = (string)($E_e*100).'%';	//数据库中必须为字符串形式
		...
		$result = $db->query("UPDATE `physics_course_potentioneter` SET `E_e`='$E_e',`U_ab`='$U_ab',`U_0`='$U_0',`I_s`='$I_s',`Rab`='$Rab',`Is`='$Is'`U0`='$U0',`Uab`='$Uab',`E`='$E' WHERE `group_num`={$group_num}");
		...
		$result = $db->query("UPDATE `physics_course_potentioneter` SET `Lx1`='$Lx1',`Lx2`='$Lx2',`Lx3`='$Lx3',`Lx4`='$Lx4',`Lx5`='$Lx5',`Lx6`='$Lx6',`Lx_ave`='$Lx_ave' WHERE `group_num`={$group_num}");
